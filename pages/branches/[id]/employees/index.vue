...
<script setup>
    ...
    // Function to initialize abilitiesCheckededit
    ...
    const selectedAbilitiesAdd = computed(() => {
        return Object.keys(abilitiesCheckedAdd.value).filter(key => abilitiesCheckedAdd.value[key]);
    });
    ...
    const checkSelections = () => {
        const addSelected = selectedAbilitiesAdd.value.length > 0;
    ...
    const addEmployeeFun = () => {
        ...
        fd.append('abilities', JSON.stringify(selectedAbilitiesAdd.value));
        validate();
    ...
    watch(editemployee, async (newValue) => {
        if (!newValue) {
            editobject.value.image = [];
            abilitiesCheckededit.value = {};
        }
    });
